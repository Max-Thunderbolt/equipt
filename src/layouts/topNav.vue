<template>
  <nav>
    <div class="nav-container">
      <div class="nav-logo">
        <router-link to="/">
          <img src="../assets/equiptBanner.png" alt="equipt. logo" class="nav-logo-img">
        </router-link>
      </div>
      <div class="nav-links">
        <div class="nav-link-item">
          <button class="nav-link-button-projects" @click="toggleProjects">
            Projects <v-icon size="16">mdi-chevron-down</v-icon>
          </button>
          <ProjectsModal v-if="isProjectsOpen" :open="isProjectsOpen" @close="closeProjects" />
        </div>
        <div class="nav-link-item">
          <button class="nav-link-button">Explore</button>
        </div>
      </div>
      <div class="profile">
        <v-btn class="profile-button" @click="toggleProfile"><v-icon>mdi-account</v-icon></v-btn>
